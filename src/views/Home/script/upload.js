...
/**
 * @Date         : 2022-03-28 17:41:13
 * @Description  :
 * @Autor        : Qzr(z5021996@vip.qq.com)
 * @LastEditors  : Qzr(z5021996@vip.qq.com)
 * @LastEditTime : 2022-03-29 09:24:03
 */
...
(function() {
  ...
  $(function() {
    ...
    // 图片容器
    let $queue = $('<ul class="filelist"></ul>')
      .appendTo($wrap.find('.queueList'))

    // 状态栏，包括进度和控制按钮
    let $statusBar = $wrap.find('.statusBar')

    // 文件总体选择信息。
    let $info = $statusBar.find('.info')

    // 上传按钮
    let $upload = $wrap.find('.uploadBtn')

    // 没选择文件之前的内容。
    let $placeHolder = $wrap.find('.placeholder')

    let $progress = $statusBar.find('.progress').hide()
    ...
    let supportTransition = (function() {
